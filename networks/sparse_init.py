...
def _lecun_uniform(fan_in: float, dtype: DTypeLikeInexact = jnp.float_):
    # handle fan_in <= 0 to avoid division by zero during scale computation
    safe_fan_in = fan_in if (fan_in and fan_in > 0) else 1.0
    scale = jnp.sqrt(1.0 / safe_fan_in)
...
def sparse_init(sparsity=0.9, dtype=jnp.float32) -> Initializer:
    ...
    def init(key: KeyArray, shape: core.Shape, dtype: DTypeLikeInexact = dtype) -> Array:
        if len(shape) == 2:
            ...
            is_conv = False
        elif len(shape) == 3:  # Conv1D or certain conv configurations
            ...
            raise ValueError(f"Unknown weight shape: {shape} with {len(shape)} dimensions")

        # If fan_in is zero, return initialized weights without attempting masking
        if fan_in == 0:
            key, init_key = jax.random.split(key)
            initializer = _lecun_uniform(1.0)  # safe initializer
            return initializer(init_key, shape, dtype)

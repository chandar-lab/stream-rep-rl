...
import distrax
import functools
from networks.MLP import MLP, sparse_init

#######################################
# Seperate Actor and Critic Networks (PPO Nets) #
#######################################

act_funcs = {"relu": nn.relu, "tanh": nn.tanh, "leaky_relu": nn.leaky_relu}
...
class Actor(nn.Module):
    ...
    def __call__(self, x):
        ...
        actor_mean = nn.Dense(
            self.action_dim, kernel_init=self.kernel_init_last, bias_init=constant(0.0)
        )(actor_mean)
        if self.cont:
...
class Critic(nn.Module):
    ...
    def __call__(self, x):
        ...
        critic = nn.Dense(
            1, kernel_init=self.kernel_init_last, bias_init=constant(0.0)
        )(critic)

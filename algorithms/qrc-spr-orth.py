...
def update_step_qrc_agent(
    ...
    if traj_buffer is not None and rng is not None:
        ...
        # Update momentum
        beta = 0.99  # Momentum factor
        spr_momentum_online = update_momentum(
            spr_momentum_online, spr_grads_online, beta
        )
        if not config.shared_online_proj:
            spr_momentum_proj = update_momentum(spr_momentum_proj, spr_grads_proj, beta)
        spr_momentum_pred = update_momentum(spr_momentum_pred, spr_grads_pred, beta)
        spr_momentum_trans = update_momentum(spr_momentum_trans, spr_grads_trans, beta)

...
def update_step_streamq(agent_state, transition, terminated, truncated, is_nongreedy):
    ...
    metrics = {
        "td_error": td_error,
        "q_val": q_taken,
        # "q_update_l2": optax.global_norm(updates), # Helpful for debugging
    }
    return (AgentState(config, train_state), metrics)
...
def experiment(args: Args, agent: Agent, run_name: str):
    ...
    if args.resume:
        ...
        if os.path.exists(checkpoint_path):
            ...
            # Restore agent state
            new_params = flax.serialization.from_bytes(
                agent_state.train_state.params, checkpoint_data["params"]
            )
            new_opt_state = flax.serialization.from_bytes(
                agent_state.train_state.opt_state, checkpoint_data["opt_state"]
            )

            new_train_state = agent_state.train_state.replace(
                params=new_params, opt_state=new_opt_state, step=checkpoint_data["step"]
            )
            agent_state = AgentState(
                agent_config=agent_state.agent_config, train_state=new_train_state
            )
    ...
    for t in range(start_step, args.total_timesteps):
        ...
        if t % 1000 == 0 and t > 0:
            ...
            # Handle case where return_queue might be empty at start
        ...
        if (t % checkpointing_frequency == 0 or t % 10000 == 0) and t > 0:
            ...
            params_bytes = flax.serialization.to_bytes(agent_state.train_state.params)
            opt_state_bytes = flax.serialization.to_bytes(
                agent_state.train_state.opt_state
            )
...
def main(
    ...
    if args.resume:
        ...
        if os.path.exists(checkpoint_path):
            ...
            # We don't load the full checkpoint here, just peek if needed,
            # or rely on experiment() to load it.
            # But we need wandb_id for wandb.init.
            with open(checkpoint_path, "rb") as f:
                # We expect the first part of pickle or a specific dict to contain metadata
                # Assuming the whole state fits in memory easily (it should for Atari)
                checkpoint_data = pickle.load(f)
